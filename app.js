(function(){
  ...
  var ACCEPT_EXT = { pptx:1, pptm:1, ppsx:1, ppsm:1, potx:1, potm:1, ppt:1 };
  ...
  /* buildRendered() provided by the PPTX adapter above */
  ...
  function show(data, name){
    currentName = name || ""; rawText = ""; parseErr = ""; hasRendered = true;
    ...
    dataView.innerHTML = ""; dataView.className = "dataview pptx";
    var loading = el("div", "pptx-msg"); loading.textContent = "Rendering slides…"; dataView.appendChild(loading);
    var ext = extOf(name);
    if (ext === "ppt"){ legacyNotice(ext); return; }
    if (typeof JSZip === "undefined"){ pptxError("ZIP reader unavailable"); return; }
    JSZip.loadAsync(data)
      .then(function(zip){ return parsePptx(zip); })
      .then(function(deck){
        deckData = deck;
      ...
      .catch(function(e){ pptxError(e && e.message || e); });
  }

  function localName(node){ return node.localName || String(node.nodeName).replace(/^.*:/, ""); }
  function firstDesc(node, ln){
    var els = node.getElementsByTagNameNS ? node.getElementsByTagNameNS("*", ln) : node.getElementsByTagName(ln);
  ...
  function parseRels(xml){
    ...
    for (var i = 0; i < rs.length; i++) map[rs[i].getAttribute("Id")] = rs[i].getAttribute("Target");
    return map;
  }
  function getXfrm(spPrOwner){
    var xf = firstDesc(spPrOwner, "xfrm"); if (!xf) return null;
    var off = firstDesc(xf, "off"), ext = firstDesc(xf, "ext");
    if (!off || !ext) return null;
    return { x: +off.getAttribute("x"), y: +off.getAttribute("y"), cx: +ext.getAttribute("cx"), cy: +ext.getAttribute("cy") };
  }
  function parseSp(node){
    var xf = getXfrm(node);
    var phEl = firstDesc(node, "ph");
    var ph = phEl ? (phEl.getAttribute("type") || "body") : "";
    var tx = firstDesc(node, "txBody");
    var paras = [];
    if (tx){
      var ps = tx.getElementsByTagNameNS("*", "p");
      for (var i = 0; i < ps.length; i++){
        var pPr = null, kids = ps[i].childNodes, runs = [];
        for (var k = 0; k < kids.length; k++){
          var kn = kids[k]; if (kn.nodeType !== 1) continue;
          var kln = localName(kn);
          if (kln === "pPr") pPr = kn;
          else if (kln === "r"){
            var t = firstDesc(kn, "t"); var txt = t ? (t.textContent || "") : "";
            var rPr = firstDesc(kn, "rPr");
            var run = { t: txt, sz: 0, b: false, i: false, color: "" };
            if (rPr){
              run.sz = +rPr.getAttribute("sz") || 0;
              run.b = rPr.getAttribute("b") === "1";
              run.i = rPr.getAttribute("i") === "1";
              var clr = firstDesc(rPr, "srgbClr"); if (clr) run.color = clr.getAttribute("val") || "";
            }
            runs.push(run);
          } else if (kln === "br") runs.push({ t: "\n", sz: 0 });
        }
        var algn = pPr ? pPr.getAttribute("algn") : "";
        paras.push({ runs: runs, algn: ({ ctr: "center", r: "right", just: "justify", l: "left" })[algn] || "" });
      }
    }
    if (!paras.length && !xf) return null;
    return { type: "text", ph: ph, x: xf && xf.x, y: xf && xf.y, cx: xf && xf.cx, cy: xf && xf.cy, paragraphs: paras };
  }
  function parsePic(node, srelMap, zip, slidePath, tasks){
    var xf = getXfrm(node);
    var blip = firstDesc(node, "blip"); if (!blip) return null;
    var rid = blip.getAttributeNS(RELNS, "embed") || blip.getAttribute("r:embed") || blip.getAttribute("embed");
    var target = rid && srelMap[rid]; if (!target) return null;
    var mediaPath = resolvePath(slidePath, target);
    var shape = { type: "pic", x: xf && xf.x, y: xf && xf.y, cx: xf && xf.cx, cy: xf && xf.cy, src: "" };
    var f = zip.file(mediaPath);
    if (f){
      var mime = /\.png$/i.test(mediaPath) ? "image/png" : /\.gif$/i.test(mediaPath) ? "image/gif" :
                 /\.svg$/i.test(mediaPath) ? "image/svg+xml" : "image/jpeg";
      tasks.push(f.async("base64").then(function(b64){ shape.src = "data:" + mime + ";base64," + b64; }));
    }
    return shape;
  }
  function resolvePath(fromPath, target){
    var base = fromPath.replace(/[^/]+$/, "");           // ppt/slides/
  ...
  function parseSlide(sxml, srelMap, zip, slidePath){
    var d = new DOMParser().parseFromString(sxml, "application/xml");
    var trees = d.getElementsByTagNameNS("*", "spTree");
    var shapes = [], tasks = [];
    if (trees.length){
      var kids = trees[0].childNodes;
      for (var i = 0; i < kids.length; i++){
        var node = kids[i]; if (node.nodeType !== 1) continue;
        var ln = localName(node);
        if (ln === "sp"){ var s = parseSp(node); if (s) shapes.push(s); }
        else if (ln === "pic"){ var p = parsePic(node, srelMap, zip, slidePath, tasks); if (p) shapes.push(p); }
      }
    }
    return Promise.all(tasks).then(function(){ return { shapes: shapes }; });
  }
  function parsePptx(zip){
    var presFile = zip.file("ppt/presentation.xml");
    if (!presFile) throw new Error("not a PowerPoint file (no presentation.xml)");
    return presFile.async("string").then(function(presXml){
      var pd = new DOMParser().parseFromString(presXml, "application/xml");
      var sz = firstDesc(pd, "sldSz");
      var W = sz ? +sz.getAttribute("cx") : 9144000, H = sz ? +sz.getAttribute("cy") : 6858000;
      var idEls = pd.getElementsByTagNameNS("*", "sldId"), rids = [];
      for (var i = 0; i < idEls.length; i++)
        rids.push(idEls[i].getAttributeNS(RELNS, "id") || idEls[i].getAttribute("r:id"));
      var relsFile = zip.file("ppt/_rels/presentation.xml.rels");
      return (relsFile ? relsFile.async("string") : Promise.resolve("")).then(function(relsXml){
        var relMap = parseRels(relsXml);
        var slides = [], chain = Promise.resolve();
        rids.forEach(function(rid, idx){
          var target = relMap[rid]; if (!target) return;
          var path = resolvePath("ppt/presentation.xml", target);
          chain = chain.then(function(){
            var f = zip.file(path); if (!f) return;
            return f.async("string").then(function(sxml){
              var relsPath = path.replace(/([^/]+)$/, "_rels/$1.rels");
              var rf = zip.file(relsPath);
              return (rf ? rf.async("string") : Promise.resolve("")).then(function(srels){
                return parseSlide(sxml, parseRels(srels), zip, path).then(function(sl){ slides[idx] = sl; });
              });
            });
          });
        });
        return chain.then(function(){ return { w: W, h: H, slides: slides.filter(Boolean) }; });
      });
    });
  }
  ...
  function renderDeck(deck){
    dataView.innerHTML = ""; dataView.className = "dataview pptx";
    if (!deck.slides.length){ var m = el("div", "pptx-msg"); m.textContent = "No slides found in this presentation."; dataView.appendChild(m); return; }
    for (var i = 0; i < deck.slides.length; i++) dataView.appendChild(renderSlide(deck.slides[i], deck.w, deck.h, i + 1));
  }
  function pct(v, whole){ return (v / whole * 100) + "%"; }
  function renderSlide(slide, W, H, num){
    var wrap = el("div", "slide-wrap");
    var lbl = el("div", "slide-num"); lbl.textContent = "Slide " + num; wrap.appendChild(lbl);
    var sl = el("div", "slide"); sl.style.aspectRatio = W + " / " + H;
    for (var s = 0; s < slide.shapes.length; s++){
      var sh = slide.shapes[s];
      var box = el("div", sh.type === "pic" ? "s-pic" : "s-txt");
      if (sh.x != null && sh.cx){
        box.style.left = pct(sh.x, W); box.style.top = pct(sh.y, H);
        box.style.width = pct(sh.cx, W); box.style.height = pct(sh.cy, H);
      }
      if (sh.type === "pic"){
        if (sh.src){ var img = doc.createElement("img"); img.src = sh.src; img.alt = ""; box.appendChild(img); }
      } else {
        for (var p = 0; p < sh.paragraphs.length; p++){
          var para = sh.paragraphs[p], pel = el("p", "s-p");
          if (para.algn) pel.style.textAlign = para.algn;
          if (!para.runs.length){ pel.innerHTML = "&#8203;"; }
          for (var r = 0; r < para.runs.length; r++){
            var run = para.runs[r];
            if (run.t === "\n"){ pel.appendChild(doc.createElement("br")); continue; }
            var span = doc.createElement("span"); span.textContent = run.t;
            var sz = run.sz || (sh.ph === "title" || sh.ph === "ctrTitle" ? 3200 : 1800);
            span.style.fontSize = (sz * 12700 / W) + "cqw";
  ...
  function pptxError(msg){
    dataView.innerHTML = ""; dataView.className = "dataview pptx";
    var box = el("div", "pptx-notice");
    var h = el("div", "pptx-notice-title"); h.textContent = "Couldn’t open this presentation";
    var p = el("p"); p.textContent = "It may be corrupt or password-protected. (" + msg + ")";
    var q = el("p", "pptx-notice-sub"); q.textContent = "Your file was not uploaded anywhere.";
    box.appendChild(h); box.appendChild(p); box.appendChild(q); dataView.appendChild(box);
  }
  function legacyNotice(ext){
    dataView.innerHTML = ""; dataView.className = "dataview pptx";
    var box = el("div", "pptx-notice");
    var h = el("div", "pptx-notice-title"); h.textContent = "Can’t preview a “." + ext + "” file";
    var p = el("p"); p.textContent = "This viewer renders modern PowerPoint decks (.pptx, .pptm, .ppsx, .potx). “." + ext + "” is the older binary format — open it in PowerPoint or LibreOffice Impress and use “Save As → .pptx” to view it here.";
    var q = el("p", "pptx-notice-sub"); q.textContent = "Your file was not uploaded anywhere.";
    box.appendChild(h); box.appendChild(p); box.appendChild(q); dataView.appendChild(box);
  }
  function buildRendered(){ if (deckData) renderDeck(deckData); }

  function readFile(file){
    if (!file) return;
    if (!isAccepted(file.name)){ if (!familyRoute(file)) toast("“" + file.name + "” isn’t a supported presentation"); return; }
  ...
  /* readFile() provided by the PPTX adapter above */
  ...
  doc.addEventListener("keydown", function(e){
    if (!id("routeCard").hidden){   // §6.10: the offer card is modal — Escape closes, Tab cycles its two buttons
  ...
  // Receiver — a sibling viewer (or the hub) hands the File across via postMessage (§6.10).
  var emptySub = doc.querySelector(".empty-sub");
  var emptySubCopy = emptySub ? emptySub.textContent : "";
  var fvhRevert = null;
  function fvhRestoreSub(){
    clearTimeout(fvhRevert); fvhRevert = null;
    if (emptySub) emptySub.textContent = emptySubCopy;
  }
  ...
  if (fvh){
    ...
    if (window.opener){
      ...
      if (emptySub){           // untrusted name — textContent only, bidi-isolated (FSI…PDI)
        emptySub.textContent = "Receiving “⁨" + fvhName + "⁩”…";
        fvhRevert = setTimeout(fvhRestoreSub, 10000);
      }
    }
  }
